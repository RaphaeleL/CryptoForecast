...
```bash 
python forecast.py --coin ETH-USD --batch_size 32 --epochs 100 --agents 6 --prediction 60
```

This command will predict the price of ETH for the next 60 days using a batch size of 32 and 100 epochs for training.
...
```bash 
python forecast.py --coin ETC-USD --batch_size 32 --epochs 100 --agents 6 
```

This command will train the model on historical ETH data and then plot the model's predictions against the actual prices in the test dataset, allowing for a visual comparison of the model's predictive accuracy.
...
If you just want to see the History then use `--plot_coin` 

```bash 
python forecast.py --coin ETC-USD --plot_coin
```
...
- --plot_coin: Just Plot the given Coin from the Yahoo Finance Database.
- --reverse: Reverse the Yahoo Dataset (sometimes it's reversed..).
